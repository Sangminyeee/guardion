    ...
    try {
      ...
      if (response.statusCode == 200) {
        final authHeader =
            ...
            response.headers['Authorization'];
        print("Authorization 헤더: \\${authHeader}");
        if (authHeader != null && authHeader.startsWith('Bearer ')) {
          final token = authHeader.substring(7);
          print("✅ 로그인 성공! 토큰: \\${token}");
          globalToken = token;
          await _storage.write(key: 'jwt_token', value: token);
          // 토큰에서 userId(sub) 추출 및 저장
          final userId = extractUserIdFromToken(token);
          if (userId != null) {
            await _storage.write(key: 'user_id', value: userId);
            print('userId 저장됨: $userId');
          } else {
            print('userId를 토큰에서 추출하지 못했습니다.');
          }
          return null;
        } else {
          print("⚠️ 토큰이 Authorization 헤더에 없음 또는 형식 오류: \\${authHeader}");
          return '서버 응답에 토큰이 없습니다. 관리자에게 문의하세요.';
        }
